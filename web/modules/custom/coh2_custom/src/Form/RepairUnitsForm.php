 ...
 * @ingroup coh2_custom
 */
class RepairUnitsForm extends FormBase {
  ...
  public function getFormId() {
    return 'coh2_custom_repair_units_form';
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $results = \Drupal::entityQuery('node')
      ->sort('field_repair_unit_weight', 'ASC')
      ->condition('type', ['unit', 'building'], 'IN')
      ->condition('field_is_repair_unit', TRUE)
      ->execute();

    foreach ($results as $vid => $nid) {
      $node = node_load($nid);
      $title = $node->get('field_front_title')->value;
      $weight = $node->get('field_repair_unit_weight')->value;
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    foreach ($submission as $nid => $item) {
      $node = node_load($nid);
      $node->set('field_repair_unit_weight', $item['weight']);
      $node->save();
    }
  }
