 ...
 * @ingroup coh2_custom
 */
class MediumRangeSquadsForm extends FormBase {
  ...
  public function getFormId() {
    return 'coh2_custom_medium_range_squads_form';
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $results = \Drupal::entityQuery('node')
      ->sort('field_medium_range_squad_weight', 'ASC')
      ->condition('type', 'unit')
      ->condition('field_is_medium_range_squad', TRUE)
      ->execute();
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    foreach ($submission as $nid => $item) {
      $node = node_load($nid);
      $node->set('field_medium_range_squad_weight', $item['weight']);
      $node->save();
    }
  }
