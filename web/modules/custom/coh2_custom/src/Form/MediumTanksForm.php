 ...
 * @ingroup coh2_custom
 */
class MediumTanksForm extends FormBase {
  ...
  public function getFormId() {
    return 'coh2_custom_medium_tanks_form';
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $results = \Drupal::entityQuery('node')
      ->sort('field_medium_tanks_weight', 'ASC')
      ->condition('type', 'unit')
      ->condition('field_is_medium_tanks', TRUE)
      ->execute();
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    foreach ($submission as $nid => $item) {
      $node = node_load($nid);
      $node->set('field_medium_tanks_weight', $item['weight']);
      $node->save();
    }
  }
