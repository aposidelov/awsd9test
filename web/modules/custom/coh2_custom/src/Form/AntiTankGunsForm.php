 ...
 * @ingroup coh2_custom
 */
class AntiTankGunsForm extends FormBase {
  ...
  public function getFormId() {
    return 'coh2_custom_anti_tank_guns_form';
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $results = \Drupal::entityQuery('node')
      ->sort('field_anti_tank_guns_weight', 'ASC')
      ->condition('type', 'unit')
      ->condition('field_is_anti_tank_guns', TRUE)
      ->execute();
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    foreach ($submission as $nid => $item) {
      $node = node_load($nid);
      $node->set('field_anti_tank_guns_weight', $item['weight']);
      $node->save();
    }
  }
