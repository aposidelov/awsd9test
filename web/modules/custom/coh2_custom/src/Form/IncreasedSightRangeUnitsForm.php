 ...
 * @ingroup coh2_custom
 */
class IncreasedSightRangeUnitsForm extends FormBase {
  ...
  public function getFormId() {
    return 'coh2_custom_increased_sight_range_units_form';
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $results = \Drupal::entityQuery('node')
      ->sort('field_incr_sight_range_weight', 'ASC')
      ->condition('type', 'unit')
      ->condition('field_is_incr_right_range_unit', TRUE)
      ->execute();
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    foreach ($submission as $nid => $item) {
      $node = node_load($nid);
      $node->set('field_incr_sight_range_weight', $item['weight']);
      $node->save();
    }
  }
