 ...
 * @ingroup coh2_custom
 */
class FlamethrowerUnitsForm extends FormBase {
  ...
  public function getFormId() {
    return 'coh2_custom_flamethrower_units_form';
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $results = \Drupal::entityQuery('node')
      ->sort('field_flame_unit_weight', 'ASC')
      ->condition('type', 'unit')
      ->condition('field_is_flame_unit', TRUE)
      ->execute();
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    foreach ($submission as $nid => $item) {
      $node = node_load($nid);
      $node->set('field_flame_unit_weight', $item['weight']);
      $node->save();
    }
  }
