 ...
 * @ingroup coh2_custom
 */
class HeavyTanksForm extends FormBase {
  ...
  public function getFormId() {
    return 'coh2_custom_heavy_tanks_form';
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $results = \Drupal::entityQuery('node')
      ->sort('field_heavy_tanks_weight', 'ASC')
      ->condition('type', 'unit')
      ->condition('field_is_heavy_tanks', TRUE)
      ->execute();
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    foreach ($submission as $nid => $item) {
      $node = node_load($nid);
      $node->set('field_heavy_tanks_weight', $item['weight']);
      $node->save();
    }
  }
