 ...
 * @ingroup coh2_custom
 */
class MachineGunsForm extends FormBase {
  ...
  public function getFormId() {
    return 'coh2_custom_machine_guns_form';
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $results = \Drupal::entityQuery('node')
      ->sort('field_machine_guns_weight', 'ASC')
      ->condition('type', 'unit')
      ->condition('field_is_machine_guns', TRUE)
      ->execute();
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    foreach ($submission as $nid => $item) {
      $node = node_load($nid);
      $node->set('field_machine_guns_weight', $item['weight']);
      $node->save();
    }
  }
