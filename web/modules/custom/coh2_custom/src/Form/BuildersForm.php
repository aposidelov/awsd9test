 ...
 * @ingroup coh2_custom
 */
class BuildersForm extends FormBase {
  ...
  public function getFormId() {
    return 'coh2_custom_builders_form';
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $results = \Drupal::entityQuery('node')
      ->sort('field_builders_weight', 'ASC')
      ->condition('type', 'unit')
      ->condition('field_is_builders', TRUE)
      ->execute();
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    foreach ($submission as $nid => $item) {
      $node = node_load($nid);
      $node->set('field_builders_weight', $item['weight']);
      $node->save();
    }
  }
