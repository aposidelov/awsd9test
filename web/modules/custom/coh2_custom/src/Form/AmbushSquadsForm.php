 ...
 * @ingroup coh2_custom
 */
class AmbushSquadsForm extends FormBase {
  ...
  public function getFormId() {
    return 'coh2_custom_ambush_squads_form';
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $results = \Drupal::entityQuery('node')
      ->sort('field_ambush_squads_weight', 'ASC')
      ->condition('type', 'unit')
      ->condition('field_is_ambush_squads', TRUE)
      ->execute();
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    foreach ($submission as $nid => $item) {
      $node = node_load($nid);
      $node->set('field_ambush_squads_weight', $item['weight']);
      $node->save();
    }
  }
