 ...
 * @ingroup coh2_custom
 */
class TankDestroyersForm extends FormBase {
  ...
  public function getFormId() {
    return 'coh2_custom_tank_destroyers_form';
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $results = \Drupal::entityQuery('node')
      ->sort('field_tank_destroyers_weight', 'ASC')
      ->condition('type', 'unit')
      ->condition('field_is_tank_destroyers', TRUE)
      ->execute();
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    foreach ($submission as $nid => $item) {
      $node = node_load($nid);
      $node->set('field_tank_destroyers_weight', $item['weight']);
      $node->save();
    }
  }
