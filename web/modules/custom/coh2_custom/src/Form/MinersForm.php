 ...
 * @ingroup coh2_custom
 */
class MinersForm extends FormBase {
  ...
  public function getFormId() {
    return 'coh2_custom_miners_form';
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $results = \Drupal::entityQuery('node')
      ->sort('field_miners_weight', 'ASC')
      ->condition('type', 'unit')
      ->condition('field_is_miners', TRUE)
      ->execute();
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    foreach ($submission as $nid => $item) {
      $node = node_load($nid);
      $node->set('field_miners_weight', $item['weight']);
      $node->save();
    }
  }
