 ...
 * @ingroup coh2_custom
 */
class MedicalUnitsForm extends FormBase {
  ...
  public function getFormId() {
    return 'coh2_custom_medical_units_form';
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $results = \Drupal::entityQuery('node')
      ->sort('field_medical_unit_weight', 'ASC')
      ->condition('type', 'unit')
      ->condition('field_is_medical_unit', TRUE)
      ->execute();
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    foreach ($submission as $nid => $item) {
      $node = node_load($nid);
      $node->set('field_medical_unit_weight', $item['weight']);
      $node->save();
    }
  }
