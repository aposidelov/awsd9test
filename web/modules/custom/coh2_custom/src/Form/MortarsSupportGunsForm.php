 ...
 * @ingroup coh2_custom
 */
class MortarsSupportGunsForm extends FormBase {
  ...
  public function getFormId() {
    return 'coh2_custom_mortars_support_guns_form';
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $results = \Drupal::entityQuery('node')
      ->sort('field_mors_and_sup_guns_weight', 'ASC')
      ->condition('type', 'unit')
      ->condition('field_is_mortars_and_sup_guns', TRUE)
      ->execute();
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    foreach ($submission as $nid => $item) {
      $node = node_load($nid);
      $node->set('field_mors_and_sup_guns_weight', $item['weight']);
      $node->save();
    }
  }
