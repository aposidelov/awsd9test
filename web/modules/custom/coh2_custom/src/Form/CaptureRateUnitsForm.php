 ...
 * @ingroup coh2_custom
 */
class CaptureRateUnitsForm extends FormBase {
  ...
  public function getFormId() {
    return 'coh2_custom_capture_rate_units_form';
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $results = \Drupal::entityQuery('node')
      ->sort('field_capture_rate_weight', 'ASC')
      ->condition('type', ['unit'], 'IN')
      ->condition('field_is_capture_unit', TRUE)
      ->execute();

    foreach ($results as $vid => $nid) {
      $node = node_load($nid);
      $title = $node->get('field_front_title')->value;
      $weight = $node->get('field_capture_rate_weight')->value;
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    foreach ($submission as $nid => $item) {
      $node = node_load($nid);
      $node->set('field_capture_rate_weight', $item['weight']);
      $node->save();
    }
  }
