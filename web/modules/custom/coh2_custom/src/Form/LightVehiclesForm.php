 ...
 * @ingroup coh2_custom
 */
class LightVehiclesForm extends FormBase {
  ...
  public function getFormId() {
    return 'coh2_custom_light_vehicles_form';
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $results = \Drupal::entityQuery('node')
      ->sort('field_light_vehicles_weight', 'ASC')
      ->condition('type', 'unit')
      ->condition('field_is_light_vehicles', TRUE)
      ->execute();
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    foreach ($submission as $nid => $item) {
      $node = node_load($nid);
      $node->set('field_light_vehicles_weight', $item['weight']);
      $node->save();
    }
  }
