 ...
 * @ingroup coh2_custom
 */
class LightTanksArmoredCarsForm extends FormBase {
  ...
  public function getFormId() {
    return 'coh2_custom_light_tanks_armored_cars_form';
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $results = \Drupal::entityQuery('node')
      ->sort('field_li_tanks_arm_cars_weight', 'ASC')
      ->condition('type', 'unit')
      ->condition('field_is_light_tanks_armored_car', TRUE)
      ->execute();
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    foreach ($submission as $nid => $item) {
      $node = node_load($nid);
      $node->set('field_li_tanks_arm_cars_weight', $item['weight']);
      $node->save();
    }
  }
