 ...
 * @ingroup coh2_custom
 */
class LongRangeArtilleryForm extends FormBase {
  ...
  public function getFormId() {
    return 'coh2_custom_long_range_artillery_form';
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $results = \Drupal::entityQuery('node')
      ->sort('field_long_range_art_weight', 'ASC')
      ->condition('type', 'unit')
      ->condition('field_is_long_range_artillery', TRUE)
      ->execute();
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    foreach ($submission as $nid => $item) {
      $node = node_load($nid);
      $node->set('field_long_range_art_weight', $item['weight']);
      $node->save();
    }
  }
