 ...
 * @ingroup coh2_custom
 */
class ScoutCarsForm extends FormBase {
  ...
  public function getFormId() {
    return 'coh2_custom_scout_cars_form';
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $results = \Drupal::entityQuery('node')
      ->sort('field_scout_cars_weight', 'ASC')
      ->condition('type', 'unit')
      ->condition('field_is_scout_cars', TRUE)
      ->execute();
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    foreach ($submission as $nid => $item) {
      $node = node_load($nid);
      $node->set('field_scout_cars_weight', $item['weight']);
      $node->save();
    }
  }
