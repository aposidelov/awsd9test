 ...
 * @ingroup coh2_custom
 */
class StealthDetectorsForm extends FormBase {
  ...
  public function getFormId() {
    return 'coh2_custom_stealth_detectors_form';
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $results = \Drupal::entityQuery('node')
      ->sort('field_stealth_detectors_weight', 'ASC')
      ->condition('type', 'unit')
      ->condition('field_is_stealth_detectors', TRUE)
      ->execute();
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    foreach ($submission as $nid => $item) {
      $node = node_load($nid);
      $node->set('field_stealth_detectors_weight', $item['weight']);
      $node->save();
    }
  }
