 ...
 * @ingroup coh2_custom
 */
class SquadWipersForm extends FormBase {
  ...
  public function getFormId() {
    return 'coh2_custom_squad_wipers_form';
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $results = \Drupal::entityQuery('node')
      ->sort('field_squad_wipers_weight', 'ASC')
      ->condition('type', 'unit')
      ->condition('field_is_squad_wipers', TRUE)
      ->execute();
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    foreach ($submission as $nid => $item) {
      $node = node_load($nid);
      $node->set('field_squad_wipers_weight', $item['weight']);
      $node->save();
    }
  }
