...
class RocketLaunchersForm extends FormBase {
  ...
  public function getFormId() {
    return 'coh2_custom_rocket_launchers_form';
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $results = \Drupal::entityQuery('node')
      ->sort('field_rocket_launchers_weight', 'ASC')
      ->condition('type', 'unit')
      ->condition('field_is_rocket_launchers', TRUE)
      ->execute();
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    foreach ($submission as $nid => $item) {
      $node = node_load($nid);
      $node->set('field_rocket_launchers_weight', $item['weight']);
      $node->save();
    }
  }
